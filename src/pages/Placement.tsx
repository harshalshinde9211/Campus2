...
import { Briefcase, Brain, Code, MessageCircle, Save, Loader2, Star } from 'lucide-react';
...
const TECHNICAL_QUESTIONS = [
  ...
  { q: 'What is a deadlock in OS?', a: 'A situation where processes are unable to proceed because each is waiting for the other.', topic: 'OS' },
  { q: 'What is a pointer in C?', a: 'A variable that stores the memory address of another variable.', topic: 'C' },
  { q: 'What is the difference between == and === in JavaScript?', a: '== compares values with coercion; === compares values and types without coercion.', topic: 'JavaScript' },
...
interface InterviewRecord { id: string; title: string; message: string; link: string; created_at: string; }
...
export function Placement() {
  ...
  const loadHistory = useCallback(async () => {
    if (!session) return;
    const { data } = await api.get('/api/notifications?type=mock_interview');
    setHistory(data || []);
    setLoading(false);
  }, [session]);
          ...
          <div className="space-y-4">
            {APTITUDE_QUESTIONS.map((item, i) => (
              <Card key={i}><CardContent className="p-5">
                <div className="mb-3 flex items-start justify-between"><p className="font-medium">{item.q}</p><Badge variant="secondary">{item.subject}</Badge></div>
                <Textarea placeholder="Type your answer..." value={answers[`apt-${i}`] || ''} onChange={(e) => setAnswers({ ...answers, [`apt-${i}`]: e.target.value })} />
                <details className="mt-2 text-sm"><summary className="cursor-pointer text-primary hover:underline">Show Answer</summary><p className="mt-1 rounded-lg bg-accent p-2 text-muted-foreground">{item.a}</p></details>
              </CardContent></Card>
            ))}
          </div>
        </TabsContent>
        ...
        <TabsContent value="technical">
          <div className="space-y-4">
            {TECHNICAL_QUESTIONS.map((item, i) => (
              <Card key={i}><CardContent className="p-5">
                <div className="mb-3 flex items-start justify-between"><p className="font-medium">{item.q}</p><Badge variant="secondary">{item.topic}</Badge></div>
                <Textarea placeholder="Type your answer..." value={answers[`tech-${i}`] || ''} onChange={(e) => setAnswers({ ...answers, [`tech-${i}`]: e.target.value })} />
                <details className="mt-2 text-sm"><summary className="cursor-pointer text-primary hover:underline">Show Answer</summary><p className="mt-1 rounded-lg bg-accent p-2 text-muted-foreground">{item.a}</p></details>
              </CardContent></Card>
            ))}
          </div>
        ...
        <TabsContent value="coding">
          <div className="space-y-6">
            {CODING_TOPICS.map((topic) => (
              <Card key={topic.level}><CardContent className="p-5">
        ...
        <TabsContent value="interview">
          <div className="grid gap-6 lg:grid-cols-2">
            <Card><CardContent className="p-5">
              ...
              <Textarea placeholder="Type your answer..." value={interviewAnswer} onChange={(e) => setInterviewAnswer(e.target.value)} rows={6} className="mb-4" />
              ...
              <Button onClick={saveInterview} disabled={!interviewAnswer}><Save className="mr-2 h-4 w-4" /> Save Answer</Button>
            </CardContent></Card>
                    ...
                    <div key={h.id} className="rounded-lg border p-3">
                      <div className="mb-1 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1">
                          {Array.from({ length: parseInt(h.link) || 0 }).map((_, i) => <Star key={i} className="h-3 w-3 fill-yellow-400 text-yellow-400" />)}
                        </div>
                      </div>
